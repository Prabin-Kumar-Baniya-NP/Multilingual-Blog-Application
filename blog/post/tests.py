...
class PostViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.post1 = Post.objects.create(
            ...
            body = "In this post, we will descuss about the education system of our country",
            author = self.user1,
            status = "A",
        )
        self.post2 = Post.objects.create(
            title = "Top 10 Python Framework ........",
            body = "In thi post, we will descuss about the lastest framework of python",
            author = self.user2,
            status = "A",
        )
    ...
    def test_view_dashboard_context(self):
        ...
        posts_queryset = Post.objects.filter(status = "A").order_by("-last_updated")
        category_queryset = Category.objects.filter(status = "A").order_by("id").values("id", "name")[:4]
        self.assertEqual(response.status_code, 200)
        self.assertQuerysetEqual(response.context["posts"], posts_queryset)
        self.assertQuerysetEqual(response.context["categories"], category_queryset)
    
    def test_post_detailView(self):
        posts = Post.objects.all()
        postID_list = [post.id for post in posts]
        for id in postID_list:
            response = self.c.get(reverse("post:view-post", kwargs={"pk": id}))
            self.assertEqual(response.status_code, 200)
    ...
    def test_post_update_view(self):
        ...
        # First we test the get method
        self.c.login(username = "test_user1", password = "abcde@12345")
        response = self.c.get(reverse("post:update-post", kwargs={"pk": self.post1.id }))
        self.assertEqual(response.status_code, 200)
        # Now we test the post method
        response = self.c.post(
        ...
        self.assertEqual(self.post1.title, "Education System of My Country ........")
    ...
    def test_post_delete_view(self):
        ...
        self.c.login(username = "test_user1", password = "abcde@12345")
        response = self.c.post(reverse("post:delete-post", kwargs={"pk": self.post1.id}))
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse("post:manage-post"))
